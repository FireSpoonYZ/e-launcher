...
export function ComposerPopover({title, close, children, compact = false}: {title: string; close(): void; children: ReactNode; compact?: boolean}) {
  const t = useText();
  useEffect(() => {
  ...
  return <div className="composer-popover-layer" onPointerDown={e => e.preventDefault()} onMouseDown={e => e.preventDefault()} onClick={e => e.stopPropagation()}>
    ...
    <section className={`composer-popover${compact ? ' compact' : ''}`} role="dialog" aria-label={title}>
