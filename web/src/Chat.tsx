...
export function ConversationComposer({conversation, running, archived=false, selection, refresh, showBranch=false, onAccepted}: ConversationComposerProps) {
  ...
  const dictationGate = useRef(new LatestRequest()).current;
  // Invalidate at commit, before a late native promise can update the newly displayed composer.
  useLayoutEffect(() => () => { dictationGate.begin(); }, [conversation.id, archived]);
  ...
  const dictate = () => action.run(async () => {
    if (archived) return;
    const request = dictationGate.begin();
    await Chat.saveDraft({conversationId:conversation.id,text:draft});
    if (!dictationGate.current(request)) return;
    const result = await Device.voice({conversationId:conversation.id});
    if (dictationGate.current(request) && result.conversationId === conversation.id) setDraft(result.text);
  });
