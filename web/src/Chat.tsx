...
export function ModelSheet({conversation, disabled, close, onChange, selectModel = Chat.selectModel}: {
  conversation: Pick<Conversation, 'id'|'piSelection'>; disabled: boolean; close(): void; onChange(): Promise<void>;
  selectModel?: typeof Chat.selectModel;
}) {
  ...
  useEffect(() => { void action.run(async () => { setDefaults((await NativeSettings.settings({effective:true, conversationId:conversation.id})).settings); setProviders((await query<CatalogProvider[]>('catalog')).filter(provider => provider.auth.configured === true)); }); }, []);
  ...
  const select = (provider: string, model: string, level?: string) => action.run(async () => {
    await selectModel({conversationId:conversation.id,providerId:provider,modelId:model,thinkingLevel:level,expectedSelection:JSON.stringify(conversation.piSelection)}); await onChange();
  });
