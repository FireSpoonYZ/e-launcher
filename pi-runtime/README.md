...
`schedule-tool` and `shower-context` load the same way, as named bundled factories. `schedule_task` sends `schedule_request` / `schedule_response` to `ScheduledTasks`; a partial update merges onto the current revision and keeps the enabled state, while a stale revision, a missing task, or an invalid rule is returned as an error. No mutation is retried. `exactAlarmGranted: false` or a non-empty `schedulingError` means the definition was saved but the exact alarm is not armed. Cancellation forwards the tool abort signal and sends `schedule_cancel` once. `shower-context` uses Pi 0.85.1's `context` event: `event.messages` is the deep copy made before each model call, and `ctx.sessionManager` is read-only. Older Shower screenshot image parts are omitted from that copy. The latest screenshot, text, tool-call ids, and non-Shower or user images stay. Stored messages and attachments are not rewritten.
