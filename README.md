...
- 未识别的边缘点击/长按/拖动通过无障碍重新注入，会有延迟，不能等同于原生透传。回放期间窗口透明且不接收新触摸；手势取消/旋转/停用会取消尚未发出的回放和停留任务。已经交给系统的注入不能通过本地 Handler 撤回，最多持续3秒；失败会提示。路径回放不保留每段采样的精确速度，且只缓存最多400个点；超过上限后保留前398个点、尾段距起点最远的点和最新终点。点击按整条已采集轨迹是否超出系统触摸容差判断，拖动后回到起点仍按路径回放。
