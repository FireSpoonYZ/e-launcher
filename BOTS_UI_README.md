...
此实现以 `develop@2d76462` 为基线，已把原补丁的工作台接入 Android。默认 AI 入口为机器人工作台，另可从设置 → 机器人进入；路由 `/bots/:sessionId?`。头像、角色、定时任务和聊天数据来自原生服务，浏览器没有插件时明确报错，不使用演示数据代替。
...
- 工作台支持每个 Bot 独立选择模型和思考强度，复用完整聊天的模型面板；运行中（含等待回答）禁止更换该 Bot 的模型，不影响其他空闲 Bot。选择模型不切换原生当前会话。
- 工作台直接显示当前 Bot 的 `ask_user_question`，复用原问卷组件的单选、多选、自定义回答、预览和取消。提交绑定会话、运行和问卷三个 ID，过期回答由原生拒绝；问卷只来自当前存活的运行。
- 全局定时任务页与 Bot 内任务共用调度器。每次继续所属会话，忙碌或等待问卷时排队；旧无归属任务仅在迁移时创建一个固定专属会话，后续重复使用。全局页面新建任务须明确选择所属 Bot / 会话。
- 完整 Markdown、附件、语音和工具详情仍使用原聊天页。没有历史时间戳的旧消息显示“历史记录”，不伪造精确发送时间。
